...
class AddPlayerView: UIViewController
{
    ...
    private var weigthTextField = UITextField()
    ...
    override func viewDidLoad()
    {
        ...
        for item in [nameTextField, numberTextField, dateOfBirthTextField, heightTextField, weigthTextField, saveButton]
        {
        ...
        setView()
        setSizeVariables()
        setPositionOfViews(ratioAndView: [1:nameTextField, 2.3:numberTextField, 3.6:dateOfBirthTextField, 4.9:heightTextField, 6.2: weigthTextField, 8.3: saveButton])
        setTextFields(textFields: [nameTextField, numberTextField, dateOfBirthTextField, heightTextField, weigthTextField])
        setSaveButton()
    }
...
//            textFields[i].attributedPlaceholder = (NSAttributedString(string: titles[i], attributes: [NSAttributedString.Key.foregroundColor : UIColor.black]))
    ...
    func setSaveButton()
    {
        ...
        saveButton.addTarget(self, action: #selector(signIn(_:)), for: .touchDown)
    }
